...
2. `ml_beats_garch` is `True` **iff all** of:
   ...
   - (b) the **Hansen-SPA** consistent *p*-value over the whole set clears
     significance (`< alpha`, default `0.05`), so the composite "no model beats the
...
(stable under the noisy RV proxy, Patton 2011). Note that condition (b) means a
lucky *pairwise* DM margin alone, which can occur, e.g. XGBoost at h=1 in the
synthetic default has DM *p* near 0.015, does **not** mint a win: it is neither the
QLIKE argmin nor SPA-significant (SPA *p* near 0.50), so the function returns
`False`.
...
- **Cost.** A genuinely superior ML model on real data must clear a deliberately
  high, snooping-resistant bar before the tool will say so. We accept the higher Type
